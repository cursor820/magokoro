...
const LANG = {
  ...
  en: {
    ...
    authErrRequired: "Please enter your login ID and password-risu",
    authErrDuplicateId: "That login ID is already taken-risu. Try another one",
    authErrInvalidCredentials: "Login ID or password is incorrect-risu",
  ...
  ko: {
    ...
    navHome: "홈", navSearch: "찾기", navPost: "올리기", navMe: "마이페이지",
    chatTitle: "코코리스 AI 챗", chatSub: "대화를 기억하고, 모르는 건 인터넷에서 찾아본다리스🌐",
    chatPlaceholder: "코코리스에게 물어보기...",
    chatWelcome: "안녕! 나는 코코리스야🐿️\n선물 추천은 물론, \"〇〇의 역사를 알려줘\" 같은 질문은 인터넷에서 찾아서 대답할게리스🌐\n(챗 답변은 아직 영어로만 가능해요. 한국어 대응 준비 중이리스🙏)",
    quickExamples: ["아사히카와 추천 기념품은?", "롯카테이 추천 상품은?", "신치토세 공항 기념품은?", "하코다테 기념품 알려줘"],
    chatNewBtn: "＋ 새 채팅", chatHistoryBtn: "🕘 기록", chatDefaultTitle: "새 채팅",
    chatThinking: "생각 중리스…", chatSearchingWeb: "🌐 인터넷에서 찾는 중리스…",
    swipeHint: "← 좌우 스와이프로 탭 이동이 된다리스 →",
    ...
    previewTitle: "🎁 코코리스 체크 & 미리보기",
    previewEmpty: "위에 입력하면 여기에 미리보기가 표시되고, 코코리스가 실시간으로 체크한다리스!",
    photoBtn: "📸 사진 찍기·고르기", photoSelected: "📸 사진을 설정했다리스!（탭하면 변경）", samplePhotoBtn: "🎲 샘플 사진",
    checkPending: "리뷰를 쓰면 코코리스가 자동으로 체크한다리스!",
    checkGateNote: "⚠️ 코코리스 체크를 통과하면 올릴 수 있다리스",
    submitBtn: "올리기 🎁",
    ...
    dbSearchNoMatch: "일치하는 기념품을 못 찾았다리스…",
    otoriyoseNote: "직접 가지 못할 때도, 소중한 사람에게 바로 전할 수 있는 마음. 코코리스가 주문을 도와준다리스🎁",
    otoriyoseAd: "※가격·용량은 참고용／링크에 광고(제휴)가 포함될 수 있습니다",
    otoriyoseAmazon: "📦 주문해서 마고코로 보내기（Amazon）",
    otoriyoseRakuten: "🚪 멀리서도 전하기（라쿠텐）",
    otoriyosePostPrefix: "📦 「", otoriyosePostSuffix: "」를 멀리 있는 소중한 사람에게도 보낼 수 있다리스🎁",
    ...
    nearbyFallbackNote: "현재 위치를 가져오지 못해서 아사히카와역 주변으로 표시한다리스!",
    locateBtn: "📍 현재 위치 사용", locating: "가져오는 중…", locationSetNote: "현재 위치를 설정했다리스!",
    nearHere: "현재 위치 근처",
    feedAll: "전체", feedFollowing: "팔로잉",
    followBtn: "＋ 팔로우", followingBtn: "✓ 팔로잉",
    feedFollowingEmpty: "아직 아무도 팔로우하지 않았다리스. 마음에 드는 사람의 「＋ 팔로우」를 눌러보라리스!",
    commentsTitle: "댓글", commentPh: "댓글 쓰기…", commentSend: "보내기",
    replyBtn: "답글", replyPh: "답글 쓰기…", noComments: "아직 댓글이 없다리스. 첫 댓글을 남겨보라리스!",
    ...
    authCocorisuWelcomeBack: "어서 와요리스!",
    authLineBtn: "LINE 로그인（준비 중）",
    authIdpassBtn: "계정 만들기",
    authAnonBtn: "가볍게 둘러보기（체험）",
    authGuideMsg: "계정을 만들 때도 이름·이메일·전화번호는 전혀 필요 없다리스🐿️ 원하는 ID와 비밀번호만 정하면 끝!\n일단 구경만 하고 싶다면 「체험」으로 OK리스, 귀찮은 설정은 하나도 없다리스✨\nLINE 로그인도 곧 사용할 수 있을 예정이리스🙏",
    ...
    authErrNetwork: "서버에 연결하지 못했다리스… 잠시 후 다시 시도해달라리스",
    authErrRequired: "로그인 ID와 비밀번호를 입력해달라리스",
    authErrDuplicateId: "그 로그인 ID는 이미 사용 중이리스. 다른 ID를 시도해달라리스",
    authErrInvalidCredentials: "로그인 ID 또는 비밀번호가 올바르지 않다리스",
    ...
    authSuccessBody: "Lv.1부터 시작이리스! 바로 선물 이야기를 모으러 가자리스.",
    authSuccessCta: "시작하기", authLogout: "로그아웃",
    recoveryPresets: ["처음 키운 반려동물의 이름은?", "태어난 동네 이름은?", "좋아하는 영화 제목은?", "어릴 적 별명은?", "가장 좋아했던 선생님 이름은?"],
    levelLabel: "Lv.", xpLabel: "XP", xpGained: "+20 XP 획득했다리스!",
    rankLabel: "현재 랭크:", rankName: "기념품 마이스터", nextLevelBefore: "다음 레벨까지", memberIdLabel: "마고코로 회원 ID:",
  ...
  if (lastItem && msg.length < 30) {
    if (/どこ(で|に)?(買|売|手に入)|場所|お店|店/.test(msg)) {
      ...
      const spotNote = spots.length > 0
        ...
        : "";
      return { mode: "reply", text: ja
        ? "「" + lastItem.name + "」は、" + lastItem.shop + "（" + lastItem.region + "）で買えるりす。" + shopMapLink(lastItem, true) + "\n🚪📦 会いに行けなくても、気持ちはちゃんと届けられるりす → [Amazon](" + amazonSearchUrl(lastItem.name) + ")　[楽天](" + rakutenSearchUrl(lastItem.name) + ")" + spotNote
    ...
    <div style={{ maxWidth: "420px", margin: "40px auto", padding: "24px", background: "#fff", borderRadius: "16px", boxShadow: "0 4px 16px rgba(0,0,0,0.05)", textAlign: "center", animation: "fadeInUp 0.4s ease" }}>
      ...
      {authStep === "welcome" && (
        <div>
          ...
          <p style={{ fontSize: "13px", color: "#64748B", margin: "0 0 16px 0" }}>{t.authSubtitle}</p>
          <div style={{ display: "flex", gap: "8px", alignItems: "flex-start", textAlign: "left", background: "#F8FAFC", border: "1px solid #E2E8F0", borderRadius: "10px", padding: "10px 12px", marginBottom: "18px" }}>
            <span style={{ fontSize: "20px", flexShrink: 0 }}>🐿️</span>
            <p style={{ fontSize: "11.5px", color: "#475569", lineHeight: 1.7, margin: 0, whiteSpace: "pre-line" }}>{t.authGuideMsg}</p>
          </div>
          <div style={{ display: "flex", flexDirection: "column", gap: "12px" }}>
            ...
            <button onClick={onGotoSignup} disabled={isSubmitting} style={{ width: "100%", background: "#3D7CB8", color: "#fff", border: "none", padding: "13px", borderRadius: "10px", fontWeight: "bold", cursor: isSubmitting ? "not-allowed" : "pointer", opacity: isSubmitting ? 0.7 : 1, fontSize: "14px" }}>
              {t.authIdpassBtn}
            </button>
            <button onClick={onAnonStart} disabled={isSubmitting} style={{ width: "100%", background: "#F8FAFC", color: "#334155", border: "1px solid #CBD5E1", padding: "13px", borderRadius: "10px", fontWeight: "bold", cursor: isSubmitting ? "not-allowed" : "pointer", opacity: isSubmitting ? 0.7 : 1, fontSize: "14px" }}>
              {t.authAnonBtn}
            </button>
            <a href="#" onClick={e => { e.preventDefault(); onGotoLogin(); }} style={{ fontSize: "12px", color: "#3D7CB8", fontWeight: "bold", marginTop: "4px" }}>{t.authLoginLink}</a>
          </div>
        </div>
      )}
  ...
  return (
    <div style={{ maxWidth: "600px", margin: "0 auto", padding: "16px" }}>
      <div style={{ background: "#fff", borderRadius: "16px", padding: "24px", border: "1px solid #E2E8F0", textAlign: "center" }}>
        ...
        {editingName ? (
          <div style={{ maxWidth: "260px", margin: "0 auto 8px auto" }}>
            <input value={nameDraft} onChange={e => setNameDraft(e.target.value.slice(0, 20))} maxLength={20}
              style={{ width: "100%", padding: "8px 10px", border: "1px solid #CBD5E1", borderRadius: "8px", fontSize: "15px", textAlign: "center", boxSizing: "border-box" }} />
            <div style={{ display: "flex", gap: "6px", marginTop: "8px", justifyContent: "center" }}>
              <button onClick={handleSave} disabled={saving || !nameDraft.trim()} style={{ background: "#3D7CB8", color: "#fff", border: "none", padding: "6px 16px", borderRadius: "6px", fontSize: "12px", fontWeight: "bold", cursor: saving ? "not-allowed" : "pointer", opacity: saving ? 0.7 : 1 }}>
                {saving ? t.mySaving : t.mySave}
              </button>
              <button onClick={() => { setEditingName(false); setNameDraft(displayName || loginId || ""); }} style={{ background: "#F1F5F9", color: "#64748B", border: "none", padding: "6px 16px", borderRadius: "6px", fontSize: "12px", cursor: "pointer" }}>
                {t.myCancel}
              </button>
            </div>
          </div>
        ...
        {isGuest && !editingName && <p style={{ fontSize: "10.5px", color: "#94A3B8", margin: "0 0 4px 0" }}>{t.myGuestNameNote}</p>}
        {saveNote && <p style={{ fontSize: "11px", color: saveNote === t.myNameSaved ? "#16A34A" : "#EF4444", margin: "0 0 4px 0" }}>{saveNote}</p>}
...
const authService = {
  async signup(loginId, password, recoveryQuestion, recoveryAnswer) {
    try {
      ...
      return { ok: true, user: { uid: cred.user.uid, loginId, displayName: loginId } };
    } catch (e) {
  ...
  async login(loginId, password) {
    try {
      ...
      const cred = await signInAnonymously(fbAuth);
      return { ok: true, user: { uid: cred.user.uid, loginId, displayName: snap.data().displayName || loginId } };
    } catch (e) {
...
export default function AgeteApp() {
  ...
  return (
    <div style={{ fontFamily: t.font, background: "#F5F7FA", minHeight: "100vh", color: "#2D2926" }}>
      <Header t={t} onToggleLang={() => dispatch({ type: A.SET_LANG, payload: lang === "ja" ? "en" : lang === "en" ? "ko" : "ja" })} isLoggedIn={isLoggedIn} userLv={userLv} userXp={userXp} />
